...
import { books, loadBook } from "../../../src/ui/src/features/library/books";
import { WordCard } from "../../../src/ui/src/features/study/shared";
...
it("loads all 11 books with definitions and available bilingual examples using bundled files only", async () => {
  vi.stubGlobal("fetch", vi.fn(async (url: string) => {
    ...
    return { ok: true, json: async () => JSON.parse(readFileSync(`src/ui/public${url}`, "utf8")) };
  }));
...
it("renders a real imported definition and bilingual example", async () => {
  const data = JSON.parse(readFileSync("src/ui/public/content/ink/ink-cet4.json", "utf8"));
...
it("has no remaining missing examples and labels generated content in study", () => {
  const coverage = JSON.parse(readFileSync("src/ui/public/content/ink/coverage.json", "utf8"));
  ...
  const data = JSON.parse(readFileSync("src/ui/public/content/ink/ink-gre.json", "utf8"));
