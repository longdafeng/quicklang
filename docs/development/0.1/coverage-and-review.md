...
> 2026-09-20 文档核对：下列测试数量、覆盖率、日志及审查结论均为 2026-09-14 的历史记录，不是当前工作区重新执行的结果。当前已移除内容生成脚本，改为浏览器与桌面共用仓库中的最终词库；`init`、`build`、`dev` 不再生成内容。当前数据约定见[词库说明](./word-library-schema)。本次未重新测量覆盖率或执行测试；复现命令以本文“可复现入口与 CI”为准。
...
`.github/workflows/ci.yml` 配置为安装固定版本覆盖率工具，执行上述覆盖率任务并上传 `build/coverage/`；这是 CI 定义，不代表最新提交的 CI 已通过。`make test-coverage` 依次执行前端、Node 原生覆盖率和 Rust 默认测试集（业务行门槛 55%）；`make test-coverage-db` 先校验词库/运行时，清理 Rust 覆盖率后汇总默认与 ignored 测试（串行运行），业务行门槛 75%。文档静态验收使用 `make docs-build`，不是持续运行的 `make docs`。前端门槛：行 93%、语句 83%、函数 79%、分支 77%；Rust 业务代码行门槛：默认测试集 55%、包含数据库 75%。
