...
打开“系统设置 → 大模型设置”，新增配置并点击“填入本机 LiteLLM 配置”。该按钮会将密钥操作设为“清除”，使用鉴权网关时必须改为“设置密钥”（编辑已有配置时为“替换密钥”），再填写网关密钥并保存；随后点击“使用此配置”启用：
...
使用网关时，厂商密钥放在网关环境中，应用保存的是网关访问密钥。当前桌面版通过 AI 配置将密钥加密保存到 seekdb，主密钥由系统 Keychain 管理，使用时解密到内存；这不同于早期仅在内存中保留密钥的实现。直接连接兼容服务时，应用配置保存的是该服务的访问密钥。远程或多人部署使用 HTTPS 和受限的网关访问密钥，不向客户端分发管理密钥；虚拟密钥、预算和用量管理需要按 LiteLLM 文档额外配置。桌面版通过 Rust 发请求。当前浏览器预览不提供数据库 AI 配置管理或密钥保存，底层浏览器传输即使允许 CORS，也不代表完整 AI 设置工作流可用。旧版浏览器设置仅在用户明确点击“迁移旧配置”后导入，不自动迁移。
